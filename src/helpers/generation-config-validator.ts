...
export class GenerationConfigValidator {
	...
	static createValidateTools(options: Partial<ChatCompletionRequest> = {}) {
		...
		if (Array.isArray(options.tools) && options.tools.length > 0) {
			const functionDeclarations = options.tools.map((tool) => {
				...
				// Filter parameters for Gemini compatibility by removing keys starting with '$'
				if (parameters) {
				...
				// Create a clean function object with only valid Gemini fields
				const cleanFunction: Record<string, unknown> = {
					name: tool.function.name,
					description: tool.function.description,
				};

				// Only add parameters if they exist and are not empty after filtering
				if (parameters && Object.keys(parameters).length > 0) {
					cleanFunction.parameters = parameters;
				}
	...
	static createFinalToolConfiguration(
		...
		if (config.useCustomTools && config.customTools && config.customTools.length > 0) {
			// Filter custom tools to remove Gemini-incompatible fields
			const filteredCustomTools = config.customTools.map(tool => {
				// Create a clean function object with only valid Gemini fields
				const cleanFunction: Record<string, unknown> = {
					name: tool.function.name,
					description: tool.function.description,
				...
				if (tool.function.parameters) {
					const filteredParameters = Object.keys(tool.function.parameters)
						.filter((key) => !key.startsWith("$"))
						.reduce(
							(filtered, key) => {
								filtered[key] = tool.function.parameters![key];
								return filtered;
							},
							{} as Record<string, unknown>
						);
					...
					if (Object.keys(filteredParameters).length > 0) {
						cleanFunction.parameters = filteredParameters;
					}
				}
			...
			return {
				tools: [
					{
						functionDeclarations: filteredCustomTools
					}
				],
